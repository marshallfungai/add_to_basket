...
class Add_to_basket_Metaboxes{
	...
	add_meta_box(
		'add-to-basket_price',
		apply_filters( $this->plugin_name . '-metabox-title-additional-info', esc_html__( 'Additional Info', 'add2basket' ) ),
		array( $this, 'metabox' ),
		$this->plugin_name,
		'normal',
		'default',
...
//	);
	...
	$fields = array();
	$fields[] = array( 'a2b-key', 'text' );
	$fields[] = array( 'price', 'text' );
	$fields[] = array( 'a2b-button', 'text' );
...
//	$fields[] = array( 'job-responsibilities', 'textarea' );
 //	$fields[] = array( 'job-location', 'text' );
	...
	if ( ! is_admin() ) { return; }
	if ( 'add_to_basket' !== $post->post_type ) { return; }
	...
	if ( 'add_to_basket' !== $object->post_type ) { return $post_id; }
